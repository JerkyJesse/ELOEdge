"""NBA injury report via ESPN public API + impact scoring."""
...
INJURY_CACHE_FILE = "nba_injuries.json"
...
def fetch_injury_report():
    """
    Fetch NBA injury report from ESPN public API.
    ...
    for player_name in out_players:
        ...
        if not adv_df.empty:
            ...
            if not adv_match.empty:
                net = float(adv_match.iloc[0].get("NET_RATING", 0) or 0)
                if net > 5:
                    impact *= 1.2
                elif net < -3:
                    impact *= 0.8
...
def show_injury_report(model=None):
    ...
    div(70)
    print("  %s  (%d players listed)" % (chi("NBA INJURY REPORT"), len(injuries)))
    div(70)
    for team in sorted(by_team.keys()):
        players = by_team[team]
        out_names = [p["player"] for p in players if p.get("status") in ("Out", "Doubtful")]
