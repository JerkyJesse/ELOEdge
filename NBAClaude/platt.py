...
ISOTONIC_FILE = "nba_isotonic_scaler.json"
...
def fit_platt_scaler(probs, actuals):
    ...
    Fixes the overconfidence at 0.9+ and underconfidence at 0.1-0.2
    seen in calibration data.
    """
...
def fit_isotonic_scaler(probs, actuals, n_bins=50):
    ...
    y_sorted = y[order]
    # Bin into n_bins groups for stability
...
def apply_isotonic(raw_prob, scaler):
    ...
    # Linear interpolation with extrapolation clamped
...
BETA_SCALER_FILE = "nba_beta_scaler.json"
...
def fit_beta_scaler(probs, actuals):
    ...
    More flexible than Platt: fixes overconfident-on-favorites but not underdogs."""
    raw = np.clip(np.array(probs, dtype=float), 1e-6, 1 - 1e-6)
    y = np.array(actuals, dtype=float)
