"""Quick 9-parameter optimizer using differential_evolution."""
import sys
import os
import numpy as np
import pandas as pd
...
def run_optimizer(csv_file=GAMES_FILE, maxiter=60, popsize=30, focus="accuracy"):
    ...
    print("=" * 60)
    print("  NBA 11-PARAM OPTIMIZER (focus=%s)" % focus)
    ...
    print("  Settings saved to nba_elo_settings.json")
    return best_params
...
if __name__ == "__main__":
    ...
    popsize = int(sys.argv[3]) if len(sys.argv) > 3 else 30
    run_optimizer(focus=focus, maxiter=maxiter, popsize=popsize)
