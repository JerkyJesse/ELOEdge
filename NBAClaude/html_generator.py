...
from live_scores import fetch_live_nba_scores, fetch_today_nba_games, fetch_tomorrow_nba_games, match_abbr_to_full
...
def _top_players_html(player_df, team_full):
    ...
    for _, p in top.iterrows():
        ...
        reb = float(p.get("TRB", 0) or 0)
        lines.append(
            "<div style='margin:2px 0;font-size:12px;color:#00ff00;'>"
            "<b>%s</b> &nbsp;%.1fp / %.1fr / %.1fa</div>" % (display, pts, reb, ast)
        )
    return "".join(lines)


def generate_today_predictions_html(model, output_file="today_nba_predictions.html"):
    ...
    html += "Stats: season averages (PPG / RPG / APG)<br>\n"
...
def generate_today_predictions_txt(model, output_file="today_nba_predictions.txt"):
    games = fetch_today_nba_games()
    if not games:
    ...
    lines   = [
        "NBA PREDICTIONS - %s%s" % (date_str, platt_note),
    ...
    for g in games:
        ...
        def top3(abbr):
            ...
            if sub.empty:
                return "-"
            return "  ".join("%s %.1fp" % (abbrev_name(str(r["Player"])), float(r.get("PTS",0)))
    ...
    lines += [
        "=" * total_w,
        "F = 70%+ confidence  |  V = 60-69%  |  Stats: PPG season avg",
...
def generate_tomorrow_predictions_html(model, output_file="tomorrow_nba_predictions.html"):
    """Generate HTML prediction table for tomorrow's scheduled NBA games."""
    games = fetch_tomorrow_nba_games()
    if not games:
