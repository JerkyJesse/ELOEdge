...
def _calc_altitude_bonus(csv_file=GAMES_FILE):
    ...
    excess to Elo points.  Only teams whose home court is >= 4000 ft
    elevation get a bonus (Denver 5280 ft, Utah 4226 ft).
    """
    import math
    ALTITUDE_TEAMS = {
        "Denver Nuggets": 5280,
    ...
    try:
        ...
        for team, elevation in ALTITUDE_TEAMS.items():
            ...
            # Scale by relative altitude (Denver = full bonus, Utah = proportional)
...
def build_model(csv_file=GAMES_FILE):
    ...
    try:
        from mega_predictor import MegaPredictor
        mega_pred = MegaPredictor(
            sport="nba", csv_file=csv_file,
            elo_model_class=NBAElo, elo_settings=settings,
            player_df=player_df,
        )
        if mega_pred._available:
            model._mega_predictor = mega_pred
