"""Sweep elo_weight and XGBoost hyperparameters to find best ensemble accuracy."""
...
class TeamTracker:
    ...
    def get_features(self, team, game_date=None, is_home=True):
        ...
        home_wp = np.mean(h_res[-WINDOW:]) if len(h_res) >= 3 else 0.5
        away_wp = np.mean(a_res[-WINDOW:]) if len(a_res) >= 3 else 0.5
        venue_wp = home_wp if is_home else away_wp
        fatigue = 0
        ...
        return {"ppg": ppg, "papg": papg, "win_pct": win_pct, "avg_margin": avg_margin,
                "off_rating": ppg, "def_rating": papg, "rest_days": rest, "games_played": n,
                "pyth": pyth, "streak": float(self.streak.get(team, 0)),
                "consistency": consistency, "trend": trend,
                "venue_win_pct": venue_wp, "fatigue": float(fatigue),
                "clutch": clutch, "score_vol": score_vol, "dominance": dominance}

    def update(self, team, pts_scored, pts_allowed, won, game_date=None, is_home=True):
        ...
        if game_date is not None: self.last_date[team] = game_date
...
def build_feats(hf, af, ep, ed, pd_=0.0, dow=3, mon=6):
    ...
    return {
        ...
        "h_score_vol": hf.get("score_vol",10.0), "a_score_vol": af.get("score_vol",10.0),
...
def sweep():
    ...
    # Pre-compute Elo probs and features once
    ...
    for idx, row in games.iterrows():
        ...
        ep = model.win_prob(home, away, team_a_home=True, neutral_site=neutral,
                            calibrated=False, game_date=gd)
        ...
        dow = gd.dayofweek if gd is not None else 3
        mon = gd.month if gd is not None else 6
        gf = build_feats(hf, af, ep, ed, pd_, dow, mon)

        elo_probs.append(ep)
        elo_diffs.append(ed)
        player_diffs.append(pd_)
        actuals.append(ha)
        if gf is not None:
            feature_list.append([gf[c] for c in FEATURE_COLS])
        ...
        model.update_game(home, away, row["home_score"], row["away_score"],
                          neutral_site=neutral, game_date=gd)
    ...
    n_games = len(actuals)
    print(f"NBA SWEEP: {n_games} games loaded")

    # Pure Elo baseline
    elo_correct = sum(1 for i in range(n_games) if (elo_probs[i] >= 0.5) == (actuals[i] == 1))
    print(f"Pure Elo: {elo_correct/n_games*100:.2f}%")

    # Now sweep XGBoost configs and elo_weights
    xgb_configs = [
        ...
        {"max_depth": 5, "eta": 0.03, "subsample": 0.9, "colsample_bytree": 0.8, "min_child_weight": 3, "lambda": 1.0, "alpha": 0.0, "rounds": 300, "label": "original"},
        {"max_depth": 6, "eta": 0.02, "subsample": 0.9, "colsample_bytree": 0.8, "min_child_weight": 2, "lambda": 0.5, "alpha": 0.0, "rounds": 400, "label": "deep"},
    ...
    elo_weights = [0.3, 0.4, 0.5, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95]
    min_train = 200
    retrain_every = 50
    ...
    for xc in xgb_configs:
        # Train XGBoost walk-forward for this config
        xgb_model = None
        train_X = []
        train_y = []
        xgb_preds = [None] * n_games
        ...
        for i in range(n_games):
            if feature_list[i] is not None:
                ...
                train_X.append(feature_list[i])
                train_y.append(actuals[i])
                if len(train_X) >= min_train and len(train_X) % retrain_every == 0:
                    X = np.array(train_X)
                    y = np.array(train_y)
                    dtrain = xgb.DMatrix(X, label=y, feature_names=FEATURE_COLS)
                    params = {
                        "objective": "binary:logistic", "eval_metric": "logloss",
                        "max_depth": xc["max_depth"], "eta": xc["eta"],
        ...
        # Now test each elo_weight
        for ew in elo_weights:
            correct = 0
            for i in range(n_games):
                if xgb_preds[i] is not None:
                    prob = ew * elo_probs[i] + (1 - ew) * xgb_preds[i]
                else:
                    prob = elo_probs[i]
                if (prob >= 0.5) == (actuals[i] == 1):
                    correct += 1
            acc = correct / n_games * 100
            if acc > best_acc:
                best_acc = acc
                best_config = f"xgb={xc['label']} ew={ew}"
                print(f"  NEW BEST: {best_config} -> {acc:.2f}%")

    print(f"\nBEST NBA ENSEMBLE: {best_config} -> {best_acc:.2f}%")
    print(f"Pure Elo baseline: {elo_correct/n_games*100:.2f}%")
    print(f"Improvement: {best_acc - elo_correct/n_games*100:+.2f}%")
