...
from live_scores import fetch_live_nhl_scores, fetch_today_nhl_games, fetch_tomorrow_nhl_games, match_abbr_to_full
...
def _top_players_html(player_df, team_full):
    ...
    # Show top skaters by points (exclude goalies)
    skaters = sub[(sub.get("GAA", pd.Series(dtype=float)).fillna(0) == 0) &
                  (sub.get("SVP", pd.Series(dtype=float)).fillna(0) == 0)]
    if skaters.empty:
        skaters = sub
    top = skaters.nlargest(3, "PTS")
    lines = []
    for _, p in top.iterrows():
        ...
        a   = float(p.get("A", 0) or 0)
        lines.append(
            "<div style='margin:2px 0;font-size:12px;color:#00ff00;'>"
            "<b>%s</b> &nbsp;%.0fG / %.0fA / %.0fP</div>" % (display, g, a, pts)
        )
    return "".join(lines)


def generate_today_predictions_html(model, output_file="today_nhl_predictions.html"):
    games = fetch_today_nhl_games()
    if not games:
    ...
    html += "Stats: season totals (Goals / Assists / Points)<br>\n"
...
def generate_today_predictions_txt(model, output_file="today_nhl_predictions.txt"):
    games = fetch_today_nhl_games()
    if not games:
    ...
    lines   = [
        "NHL PREDICTIONS - %s%s" % (date_str, platt_note),
    ...
    for g in games:
        ...
        def top3(abbr):
            if player_df.empty:
                return "-"
            sub = player_df[player_df["Tm"]==abbr].nlargest(3,"PTS")
            if sub.empty:
                return "-"
            return "  ".join("%s %.0fP" % (abbrev_name(str(r["Player"])), float(r.get("PTS",0)))
    ...
    lines += [
        "=" * total_w,
        "F = 70%+ confidence  |  V = 60-69%  |  Stats: season total Points",
...
def generate_tomorrow_predictions_html(model, output_file="tomorrow_nhl_predictions.html"):
    """Generate HTML prediction table for tomorrow's scheduled NHL games."""
    games = fetch_tomorrow_nhl_games()
    if not games:
