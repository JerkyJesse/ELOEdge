...
def run_mega_backtest(csv_file, sport="nfl", elo_model_class=None,
    ...
    hmm = None
    if _on("hmm"):
        n_hmm_states = mp.get("hmm_states", 2 if sport in ("nfl",) else 3)
        hmm = LeagueHMM(n_states=n_hmm_states, min_games=defaults["hmm_min_games"])
    ...
    meta = MetaLearner(sport, meta_model=meta_type)
