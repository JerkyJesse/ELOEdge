"""Blogger HTML and plain-text table generation for today's NFL games."""
...
from live_scores import fetch_live_nfl_scores, fetch_today_nfl_games, fetch_tomorrow_nfl_games, match_abbr_to_full


def _top_players_html(player_df, team_full):
    """Show top offensive players (passing/rushing/receiving) for HTML output."""
    ...
    # Sort by a composite score: passing yards most valuable, then rushing, then receiving
    sub = sub.copy()
    sub["_sort"] = (
        pd.to_numeric(sub.get("PassYds", 0), errors="coerce").fillna(0) * 1.0 +
        pd.to_numeric(sub.get("RushYds", 0), errors="coerce").fillna(0) * 0.8 +
        pd.to_numeric(sub.get("RecYds", 0), errors="coerce").fillna(0) * 0.7
    )
    top   = sub.nlargest(3, "_sort")
    lines = []
    for _, p in top.iterrows():
        parts   = str(p.get("Player","-")).strip().split()
        display = ("%s. %s" % (parts[0][0], " ".join(parts[1:]))) if len(parts) >= 2 else str(p.get("Player","-"))
        pass_yds = float(p.get("PassYds", 0) or 0)
        rush_yds = float(p.get("RushYds", 0) or 0)
        rec_yds  = float(p.get("RecYds", 0) or 0)
        # Show the most relevant stat
        if pass_yds > rush_yds and pass_yds > rec_yds:
            stat_str = "%.0f pass" % pass_yds
        elif rush_yds > rec_yds:
            stat_str = "%.0f rush" % rush_yds
        else:
            stat_str = "%.0f rec" % rec_yds
        lines.append(
            "<div style='margin:2px 0;font-size:12px;color:#00ff00;'>"
            "<b>%s</b> &nbsp;%s</div>" % (display, stat_str)
        )
    return "".join(lines)


def generate_today_predictions_html(model, output_file="today_nfl_predictions.html"):
    ...
    html += "Stats: season leaders (passing / rushing / receiving)<br>\n"
...
def generate_today_predictions_txt(model, output_file="today_nfl_predictions.txt"):
    games = fetch_today_nfl_games()
    if not games:
    ...
    lines   = [
        "NFL PREDICTIONS - %s%s" % (date_str, platt_note),
    ...
    for g in games:
        ...
        def top3(abbr):
            if player_df.empty:
                return "-"
            sub = player_df[player_df["Tm"]==abbr]
            if sub.empty:
                return "-"
            sub = sub.copy()
            sub["_s"] = (
                pd.to_numeric(sub.get("PassYds", 0), errors="coerce").fillna(0) +
                pd.to_numeric(sub.get("RushYds", 0), errors="coerce").fillna(0) * 0.8 +
                pd.to_numeric(sub.get("RecYds", 0), errors="coerce").fillna(0) * 0.7
            )
            sub = sub.nlargest(3, "_s")
            return "  ".join("%s" % abbrev_name(str(r["Player"]))
    ...
    lines += [
        "=" * total_w,
        "F = 70%+ confidence  |  V = 60-69%  |  Stats: season leaders",
...
def generate_tomorrow_predictions_html(model, output_file="tomorrow_nfl_predictions.html"):
    """Generate HTML prediction table for tomorrow's scheduled NFL games."""
    games = fetch_tomorrow_nfl_games()
    if not games:
