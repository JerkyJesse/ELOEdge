#!/usr/bin/env python3
"""
NFL ELOEdge -- Elo-only predictions + Predicts $1 contract tracker.
Weather, division rivalry, QB injury impact, Platt-calibrated.
"""
...
# Optional data sources (odds / weather / kalshi)
import sys
try:
...
try:
    from advanced_stats import (load_or_download_epa, show_epa_rankings,
                                get_advanced_features as get_nfl_advanced_features,
                                download_officials_data, compute_referee_tendencies)
    ...
    # ── Data commands ──────────────────────────────────────────────────
        ...
        if HAS_DATA_MODULES:
            ...
            if team:
                ...
                if found:
                    show_weather_report(found, "nfl")
                else:
                    print(cerr("Team not found: %s" % team))
        else:
            print(cerr("Weather module not available."))
    elif cmd in ("advstats", "epa"):
        if HAS_ADVANCED_STATS:
            print(cdim("  Downloading NFL play-by-play EPA data (may take a minute)..."))
            epa_df, rolling = load_or_download_epa()
            show_epa_rankings(rolling)
        else:
        ...
        if HAS_ADVANCED_STATS:
            ...
            try:
                ...
                from advanced_stats import download_pbp_data, compute_referee_tendencies
                pbp = download_pbp_data()
                officials = download_officials_data()
                if officials is not None and pbp is not None:
                    ref_stats = compute_referee_tendencies(officials, pbp)
                    if ref_stats:
                        print(f"\n  {'Referee':<25} {'Games':>6} {'Home Win%':>10} {'Avg Total':>10}")
                        print("  " + "-" * 55)
                        for ref, stats in sorted(ref_stats.items(), key=lambda x: x[1]["home_win_rate"], reverse=True):
                            print(f"  {ref:<25} {stats['games']:>6} {stats['home_win_rate']:>9.1%} {stats['avg_total_score']:>10.1f}")
                    else:
                        print(cwarn("  No referee tendency data computed."))
                else:
                    print(cerr("  Failed to download officials or PBP data."))
            except Exception as e:
                print(cerr("  Officials analysis failed: %s" % e))
        else:
            print(cerr("Advanced stats module not available."))
...
SETTINGS (39 Elo params, type 'set' to see all):
  ...
  set travel=15 | set pace=35 | set sos=20 | set streak=10 | set bye=15
...
""" % chi("backtest"))
    ...
    if injuries:
        out_count = sum(1 for i in injuries if i.get("status") in ("Out", "Doubtful",
                                                                     "Injured Reserve"))
        print(cdim("  Injuries loaded: %d players (%d Out/Doubtful/IR)" % (len(injuries), out_count)))
    if not csv_file:
    ...
    while True:
        ...
        if cmd in ("today","html","blog","blogger"):
            print("\n" + chi("Generating today's NFL predictions HTML..."))
            generate_today_predictions_html(model)
            continue

        if cmd == "tomorrow":
            print("\n" + chi("Generating tomorrow's NFL predictions HTML..."))
            generate_tomorrow_predictions_html(model)
            continue
        ...
        if cmd not in KNOWN_CMDS and not cmd.startswith("set ") and cmd.strip():
            ...
            if team_a:
                ...
                if HAS_KALSHI and settings_now.get("auto_kalshi"):
                    ...
                    kalshi = find_kalshi_odds(h_team, a_team, "nfl")
