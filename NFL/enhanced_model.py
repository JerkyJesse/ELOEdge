...
ENHANCED_MODEL_FILE = "nfl_enhanced_model.json"
WINDOW = 5  # rolling window for team stats (smaller for NFL's 17-game season)
PYTH_EXP = 2.37  # NFL Pro Football Reference Pythagorean exponent
...
class TeamTracker:
    ...
    def get_features(self, team, game_date=None, is_home=True):
        ...
        # Pythagorean win expectation (PFR formula for NFL)
        ...
        consistency = float(np.std(margins[-WINDOW:])) if len(margins) >= 3 else 14.0

        # Trend: recent 3 vs full window win% (NFL has fewer games)
        recent_n = min(3, len(results))
        recent_wl = np.mean(results[-recent_n:]) if recent_n >= 2 else win_pct
        trend = recent_wl - win_pct
        ...
        # New features: B2B (short week), road trip, margin trend, ultra-recent form
        is_b2b = 1.0 if (rest is not None and rest <= 5) else 0.0
        ...
        avg_opp_elo = float(np.mean(opp_elos[-5:])) if len(opp_elos) >= 3 else 1500.0
        ...
        games_played = n
        pyth_regression_risk = abs(pyth_residual) * min(1.0, games_played / 8.0)
        ...
        # Rest quality (quadratic): U-shaped curve, 7 days optimal for NFL
        rest_quality = 0.0
        if rest is not None:
            rest_quality = 1.0 - ((rest - 7.0) ** 2) / 49.0
    ...
    def update(self, team, pts_scored, pts_allowed, won, game_date=None,
        ...
        # Track close games (1-possession margin <= 7) and blowouts (3+ scores >= 17)
...
def compute_team_stats(player_df, adv_df=None):
    """Aggregate player-level stats to team-level averages (NFL-specific)."""
    ...
    for abbr in player_df["Tm"].unique():
        ...
        stats = {}
        # NFL-specific: passing yards, rushing yards, TDs
        for col, key in [("Pass Yds", "pass_yds"), ("Rush Yds", "rush_yds"),
                         ("Pass TD", "pass_td"), ("Rush TD", "rush_td"),
                         ("Yds", "total_yds")]:
            if col in tp.columns:
                vals = pd.to_numeric(tp[col], errors="coerce").dropna()
                stats[key] = float(vals.sum()) if len(vals) > 0 else 0.0
            else:
                stats[key] = 0.0
        # TD rate (TDs per player)
        total_tds = stats.get("pass_td", 0) + stats.get("rush_td", 0)
        n_players = max(len(tp), 1)
        stats["td_rate"] = total_tds / n_players
        # Advanced stats
...
def build_game_features(home_feats, away_feats, elo_prob, elo_diff,
                        player_diff=0.0, day_of_week=0, month=9,
    ...
    return {
        ...
        "h_rest": home_feats["rest_days"] if home_feats["rest_days"] is not None else 7.0,
        "a_rest": away_feats["rest_days"] if away_feats["rest_days"] is not None else 7.0,
        "rest_diff": (home_feats["rest_days"] or 7) - (away_feats["rest_days"] or 7),
        ...
        # --- NFL-specific player stat features ---
        "h_pass_yds": hs.get("pass_yds", 0.0),
        "a_pass_yds": as_.get("pass_yds", 0.0),
        "pass_yds_diff": hs.get("pass_yds", 0.0) - as_.get("pass_yds", 0.0),
        "h_rush_yds": hs.get("rush_yds", 0.0),
        "a_rush_yds": as_.get("rush_yds", 0.0),
        "rush_yds_diff": hs.get("rush_yds", 0.0) - as_.get("rush_yds", 0.0),
        "h_td_rate": hs.get("td_rate", 0.0),
        "a_td_rate": as_.get("td_rate", 0.0),
        "td_rate_diff": hs.get("td_rate", 0.0) - as_.get("td_rate", 0.0),
        # Advanced occult features
...
FEATURE_COLS = [
    ...
    "total_points_ou",
    # NFL-specific player stat features
    "h_pass_yds", "a_pass_yds", "pass_yds_diff",
    "h_rush_yds", "a_rush_yds", "rush_yds_diff",
    "h_td_rate", "a_td_rate", "td_rate_diff",
    # Advanced occult features
...
def run_enhanced_backtest(csv_file=GAMES_FILE, min_train=200, retrain_every=50,
    ...
    for idx, row in tqdm(games.iterrows(), total=len(games),
        ...
        dow = game_date.dayofweek if game_date is not None else 0
        mon = game_date.month if game_date is not None else 9
...
def shap_feature_importance(xgb_model=None, feature_rows=None):
    if xgb_model is None:
        ...
        if xgb_model is None:
            ...
            return None
        if feature_rows is None and os.path.exists("nfl_enhanced_features.npz"):
            data = np.load("nfl_enhanced_features.npz")
...
def save_enhanced_model(result, filename=ENHANCED_MODEL_FILE):
    if result is None or result.get("xgb_model") is None:
        return
    result["xgb_model"].save_model("nfl_xgb_model.json")
    payload = {
    ...
    if result.get("feature_rows"):
        np.savez_compressed("nfl_enhanced_features.npz",
                            features=np.array(result["feature_rows"]),
                            labels=np.array(result["label_rows"]))
    logging.info("Enhanced model saved -> %s + nfl_xgb_model.json", filename)


def load_enhanced_model(filename=ENHANCED_MODEL_FILE):
    if not os.path.exists(filename) or not os.path.exists("nfl_xgb_model.json"):
        return None, None
    ...
    xgb_model.load_model("nfl_xgb_model.json")
    return xgb_model, meta


if __name__ == "__main__":
    import sys
    ew = float(sys.argv[1]) if len(sys.argv) > 1 else 0.9
