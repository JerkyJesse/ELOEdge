"""Regression tests for ELO-only post-strip NBA model."""
...
def test_init_no_stripped_fields():
    ...
    for attr in ("_xgb_model", "_xgb_meta", "_mega_predictor", "_mega_loading"):
        assert not hasattr(m, attr), f"{attr} should be stripped from NBAElo"
...
def test_pick_winner_returns_valid_tuple():
    # With empty team history, win_prob's rolling adjustments may swing sharply.
    # Verify only that pick_winner returns a valid (winner, prob) tuple without error.
    m = _model()
    m.ratings["A"] = 1700
    m.ratings["B"] = 1500
    winner, prob = m.pick_winner("A", "B", team_a_home=True, neutral_site=False)
    assert winner in ("A", "B")
    assert 0.5 <= prob <= 1.0  # pick_winner always returns the winning side's prob
...
def test_show_settings_no_nameerror(capsys):
    ...
    assert "Base Rating" in captured.out, "show_settings should still print Elo rows"
