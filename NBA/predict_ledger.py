...
from live_scores import fetch_live_nba_scores
...
def _fetch_live_game_status(home_team, away_team):
    try:
        ...
        for g in games:
            ht = g.get("homeTeam", {}).get("teamTricode", "").upper()
            at = g.get("awayTeam", {}).get("teamTricode", "").upper()
            if {ht, at} != {home_abbr, away_abbr}:
                continue
            status_text = str(g.get("gameStatusText", "")).strip()
            game_status = int(g.get("gameStatus", 0))
            if game_status == 1:
                return "TIPOFF %s" % status_text
            elif game_status == 2:
                period    = g.get("period", "?")
                clock     = g.get("gameClock", "")
                ht_pts    = g.get("homeTeam", {}).get("score", "?")
                at_pts    = g.get("awayTeam", {}).get("score", "?")
                clock_str = clock.lstrip("PT").replace("M", ":").replace("S", "").rstrip(":")
                return "Q%s %s | %s %s-%s %s" % (period, clock_str, at, at_pts, ht_pts, ht)
            elif game_status == 3:
                ht_pts = g.get("homeTeam", {}).get("score", "?")
                at_pts = g.get("awayTeam", {}).get("score", "?")
                return "FINAL %s %s-%s %s" % (at, at_pts, ht_pts, ht)
        return None
    except Exception:
        return None
