"""Live NBA scores fetching and open-trade tracking display."""
...
LIVE_SCORES_URL = "https://cdn.nba.com/static/json/liveData/scoreboard/todaysScoreboard_00.json"
SCHEDULE_URL    = "https://cdn.nba.com/static/json/staticData/scheduleLeagueV2.json"


def _fetch_schedule_for_date(target_date):
    """Fetch scheduled NBA games for a specific date from the NBA CDN schedule."""
    try:
        ...
        for d in dates:
            if date_str in d.get("gameDate", ""):
                results = []
                for g in d.get("games", []):
                    home = g.get("homeTeam", {})
                    away = g.get("awayTeam", {})
                    results.append({
                        "home_team_city": (home.get("teamCity", "") + " " + home.get("teamName", "")).strip(),
                        "home_abbr":      home.get("teamTricode", ""),
                        "away_team_city": (away.get("teamCity", "") + " " + away.get("teamName", "")).strip(),
                        "away_abbr":      away.get("teamTricode", ""),
                        "home_score":     0,
                        "away_score":     0,
                        "status":         g.get("gameStatusText", "").strip() or "Scheduled",
                        "period":         0,
                    })
                return results
        return []
...
def fetch_today_nba_games():
    """Fetch today's scheduled NBA games from the NBA CDN schedule."""
    return _fetch_schedule_for_date(datetime.now())


def fetch_tomorrow_nba_games():
    """Fetch tomorrow's scheduled NBA games from the NBA CDN schedule."""
    return _fetch_schedule_for_date(datetime.now() + timedelta(days=1))


def fetch_live_nba_scores():
    try:
        req = urllib.request.Request(
            LIVE_SCORES_URL,
            headers={"User-Agent": "Mozilla/5.0", "Accept": "application/json"},
        ...
        for g in games:
            ...
            away = g.get("awayTeam", {})
            results.append({
                "home_team_city": home.get("teamCity","") + " " + home.get("teamName",""),
                "home_abbr":      home.get("teamTricode",""),
                "away_team_city": away.get("teamCity","") + " " + away.get("teamName",""),
                "away_abbr":      away.get("teamTricode",""),
...
def _print_live_scores_once(model):
    ...
    live    = fetch_live_nba_scores()
    div(100)
    print("  " + chi("LIVE NBA SCORES - TODAY'S GAMES"))
    div(100)
    if not live:
