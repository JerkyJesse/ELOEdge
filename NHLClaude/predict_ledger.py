...
from live_scores import fetch_live_nhl_scores
...
def _fetch_live_game_status(home_team, away_team):
    """Fetch live game status via ESPN NHL scoreboard."""
    try:
        games = fetch_live_nhl_scores()
        ...
        for g in games:
            ...
            if is_final:
                return "FINAL %s %s-%s %s" % (at, g["away_score"], g["home_score"], ht)
            period = g.get("period", 0)
            if period > 0:
                return "P%s | %s %s-%s %s" % (period, at, g["away_score"], g["home_score"], ht)
...
def show_open_lots(df):
    ...
    for i, (_, r) in enumerate(open_df.iterrows()):
        ...
        elif "TODAY" in gs_str or "P" in gs_str[:2]:
