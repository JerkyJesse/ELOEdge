...
def _calc_altitude_bonus(csv_file=GAMES_FILE):
    ...
    excess to Elo points.  Only teams whose home arena is >= 4000 ft
    elevation get a bonus (Colorado Avalanche 5280 ft, Utah Hockey Club 4226 ft).
    """
    import math
    ALTITUDE_TEAMS = {
        "Colorado Avalanche": 5280,
        "Utah Hockey Club": 4226,
    }
    ...
    try:
        ...
        for team, elevation in ALTITUDE_TEAMS.items():
            ...
            # Scale by relative altitude (Denver = full bonus)
            altitude_scale = (elevation - 4000) / (5280 - 4000)
            elo_bonus *= min(1.0, altitude_scale)
...
def _populate_game_history(model, csv_file=GAMES_FILE):
    ...
    try:
        ...
        for _, row in games.iterrows():
            ...
            # Score history for pace estimation
...
def build_model(csv_file=GAMES_FILE):
    ...
    if model.load():
        ...
        t = threading.Thread(
            ...
            args=(model, "nhl", csv_file, NHLElo, settings, player_df),
            daemon=True,
        )
        t.start()
        return model
        ...
        try:
            ...
            if "date" in row and pd.notna(row["date"]):
                try:
                    ...
                    row_season = dt.year + 1 if dt.month >= 10 else dt.year
                except Exception as e:
    ...
    try:
        from mega_predictor import MegaPredictor
        mega_pred = MegaPredictor(
            sport="nhl", csv_file=csv_file,
            elo_model_class=NHLElo, elo_settings=settings,
            player_df=player_df,
        )
        if mega_pred._available:
            model._mega_predictor = mega_pred
