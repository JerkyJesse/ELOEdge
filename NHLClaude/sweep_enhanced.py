"""Sweep elo_weight and XGBoost hyperparameters to find best NHL ensemble accuracy."""
...
class TeamTracker:
    ...
    def get_features(self, team, game_date=None, is_home=True):
        ...
        sv = float(np.std(scored[-WINDOW:])) if len(scored) >= 5 else 1.5
...
def build_feats(hf, af, ep, ed, pd_=0.0, gd=0.0, hg=0.0, ag=0.0, dow=3, mon=1):
    if hf is None or af is None: return None
    return {
        "elo_prob": ep, "elo_diff": ed, "player_diff": pd_,
        "goalie_diff": gd, "h_goalie": hg, "a_goalie": ag,
        ...
        "h_score_vol": hf.get("score_vol",1.5), "a_score_vol": af.get("score_vol",1.5),
...
def sweep():
    ...
    for idx, row in games.iterrows():
        ...
        if gd is not None:
            row_season = gd.year + 1 if gd.month >= 10 else gd.year
        ...
        hgr = model._goalie_ratings.get(hs, 0.0)
        agr = model._goalie_ratings.get(as_, 0.0)
        ...
        mon = gd.month if gd is not None else 1
        gf = build_feats(hf, af, ep, ed, pd_, hgr-agr, hgr, agr, dow, mon)
    ...
    print(f"NHL SWEEP: {n} games, Pure Elo: {elo_correct/n*100:.2f}%")
    ...
    print(f"\nBEST NHL ENSEMBLE: {best_config} -> {best_acc:.2f}%")
