...
class LeagueMonteCarlo:
    ...
    def __init__(self, n_simulations=5000, min_games=10, seed=42):
    ...
    def simulate_game(self, home, away):
        ...
        # Generate home team scores (average of their offense and opponent's defense)
        h_offense = h_dist.sample_score(is_home=True, n_samples=n, rng=self.rng)
        a_defense = a_dist.sample_conceded(is_home=False, n_samples=n, rng=self.rng)
        home_scores = (h_offense + a_defense) / 2

        # Generate away team scores
        a_offense = a_dist.sample_score(is_home=False, n_samples=n, rng=self.rng)
        h_defense = h_dist.sample_conceded(is_home=True, n_samples=n, rng=self.rng)
        away_scores = (a_offense + h_defense) / 2
