"""Quick 11-parameter optimizer using differential_evolution."""
import sys
import os
import numpy as np
import pandas as pd
...
_OPT_KEYS = ("k", "home_adv", "player_boost", "rest_factor", "form_weight",
              "travel_factor", "sos_factor", "pace_factor", "playoff_hca_factor",
...
def run_optimizer(csv_file=GAMES_FILE, maxiter=60, popsize=30, focus="accuracy"):
    ...
    bounds = [
        (10, 50),    # k (NFL uses higher K for 17-game season)
        (20, 100),   # home_adv (NFL HA is ~57%)
        (0, 80),     # player_boost
        (0, 80),     # rest_factor (rest matters hugely in NFL)
        (0, 30),     # form_weight
        (0, 50),     # travel_factor
        (0, 50),     # sos_factor
        (0, 60),     # pace_factor
    ...
    def objective(params):
        ...
        if acc > best_acc[0]:
            best_acc[0] = acc
            print("  [%d] NEW BEST: Acc=%.2f%% LL=%.4f Brier=%.4f" % (
    ...
    print("=" * 60)
    print("  NFL 11-PARAM OPTIMIZER (focus=%s)" % focus)
    ...
    print("  Settings saved to nfl_elo_settings.json")
    return best_params
