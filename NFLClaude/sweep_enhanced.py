"""Sweep elo_weight and XGBoost hyperparameters to find best NFL ensemble accuracy."""
...
class TeamTracker:
    ...
    def get_features(self, team, game_date=None, is_home=True):
        ...
        rs = max(ppg, 0.1) ** PYTH_EXP
        ra = max(papg, 0.1) ** PYTH_EXP
        pyth = rs / (rs + ra)
        consistency = float(np.std(margins[-WINDOW:])) if len(margins) >= 3 else 14.0
        rn = min(3, len(results))
        recent = np.mean(results[-rn:]) if rn >= 2 else win_pct
        trend = recent - win_pct
        rest = None
        ...
        sv = float(np.std(scored[-WINDOW:])) if len(scored) >= 5 else 10.0
...
def build_feats(hf, af, ep, ed, pd_=0.0, dow=0, mon=9):
    ...
    return {
        ...
        "h_rest": hf["rest_days"] if hf["rest_days"] is not None else 7.0,
        "a_rest": af["rest_days"] if af["rest_days"] is not None else 7.0,
        "rest_diff": (hf["rest_days"] or 7) - (af["rest_days"] or 7),
...
def sweep():
    ...
    for idx, row in games.iterrows():
        ...
        ep = model.win_prob(home, away, team_a_home=True, neutral_site=neutral, calibrated=False, game_date=gd)
        ...
        dow = gd.dayofweek if gd is not None else 0
        mon = gd.month if gd is not None else 9
        ...
        model.update_game(home, away, row["home_score"], row["away_score"], neutral_site=neutral, game_date=gd)
    ...
    print(f"NFL SWEEP: {n} games, Pure Elo: {elo_correct/n*100:.2f}%")
    ...
    elo_weights = [0.5, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95]
    ...
    print(f"\nBEST NFL ENSEMBLE: {best_config} -> {best_acc:.2f}%")
